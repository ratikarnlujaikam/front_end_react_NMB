...
class percen_ng extends Component {
  ...
  doGetDataReport = async () => {
    try {
      const result = await httpClient.get(
        server.percen_Downtime_URL + "/" + this.state.startDate
      );
      console.log(result);
  ...
  render() {
    ...
    const totalDT = this.state.rawData_2.length > 0 ? this.state.rawData_2[0].Total_DT : 0;
    const Second_perPcs = this.state.rawData_2.length > 0 ? this.state.rawData_2[0].Second_perPcs : 0;
    return (
          ...
          <div className="row">
            <div className="col-12">
              <div className="card">
                ...
                <h1>Downtime Dashboard Monitoring {this.state.startDate}</h1>
                <div className="col-5">
                     </div>
                          ...
                          return (
                            <div key={index} className="col-md-1">
                              <button
                                className="btn btn-warning text-center"
                                style={{
                                  backgroundColor: item.NG === null ? "green" : item.NG >20 ? "red" : "#ff9800",
                                  color: item.NG === null ? "white" : item.NG >20 ? "white" : "black",
                                  padding: "10px",
                                  margin: "8px",
