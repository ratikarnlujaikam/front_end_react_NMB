...
class Monthly_LAR_report_all_Model extends Component {
  ...
  doGetDataReport = async () => {
    try {
      ...
      let seriesData = PivotTable_month; // Assuming PivotTable_month contains series data
          ...
          chart: {
            ...
            events: {
              dataPointSelection: (event, chartContext, config) => {
                ...
                const category = this.state.xAxis_month[config.dataPointIndex];
                const value = this.state.series_month[config.seriesIndex].data[config.dataPointIndex];
                const seriesName = this.state.series_month[config.seriesIndex].name;
                this.props.history.push(`/Compare_Output?&line=${seriesName}&startDate=${this.state.start_Date_one_day}&to_link=$("ok")&from=/Yield_Monitoring_By_Team`)
